...
import { ChevronLeft, Loader2, Wallet, Globe, Landmark } from 'lucide-react';
...
function BStockDetail({ symbol }: { symbol: string }) {
  ...
  const setPct = (pct: number) => {
    ...
    if (side === 'BUY') {
      ...
      const maxQty = (budget * leverage) / (livePrice * (1 + COMMISSION_RATE));
      setQty(maxQty > 0 ? maxQty.toFixed(4) : '');
    } else {
      setQty((held * pct).toFixed(4));
    }
  };
    ...
    <div className="max-w-6xl mx-auto p-4 space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-[11px] text-muted-foreground">{info?.industry ?? '代币化美股'} · 24/7 · 瞬时结算</div>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-2 space-y-4">
          <Card className="overflow-hidden">
            ...
            <div className="h-[340px] sm:h-[400px] p-2">
              <CandleChart symbol={symbol} interval={CHART_TABS[chartTab].interval} klinesFn={bstockApi.klines} />
            </div>
          </Card>
          ...
          <Card>
            <CardContent className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                {info?.homepage && (
                  <a href={info.homepage} target="_blank" rel="noopener noreferrer" className="text-[11px] text-primary hover:underline inline-flex items-center gap-1">
                ...
                <>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {[
                      ...
                      ['52周', info.week52Low != null && info.week52High != null ? `${fmtNum(info.week52Low)}–${fmtNum(info.week52High)}` : '-'],
                    ].map(([k, v]) => (
                      <div key={k} className="rounded-xl bg-muted/40 px-3 py-2">
                        <div className="text-[10px] text-muted-foreground">{k}</div>
                        <div className="text-[13px] font-bold tabular-nums tracking-tight">{v}</div>
                      </div>
                    ))}
                  </div>
                  {info.description && <p className="text-[12px] leading-relaxed text-muted-foreground">{info.description}</p>}
          ...
          <Card className="overflow-hidden">
            <CardContent className="p-4 space-y-4">
                ...
                {(['BUY', 'SELL'] as const).map(sd => (
                  <button
                    ...
                    className={cn(
                      ...
                      side === sd
                        ? (sd === 'BUY' ? "bg-green-500/15 text-green-400 ring-1 ring-green-500/30" : "bg-red-500/15 text-red-400 ring-1 ring-red-500/30")
              ...
              <div className="flex items-center justify-between text-[11px] text-muted-foreground">
                <span className="inline-flex items-center gap-1"><Wallet className="w-3 h-3" /> 可用 <span className="text-foreground font-bold tabular-nums">{fmtNum(balance)}</span></span>
                <span>持有 <span className="text-foreground font-bold tabular-nums">{fmtNum(held)}</span></span>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-4 gap-1.5">
                  {PCTS.map(p => (
                    <Button key={p} variant="outline" size="sm" className="h-7 text-[11px]" onClick={() => setPct(p)}>
                      {p === 1 ? '全部' : `${p * 100}%`}
                    </Button>
                  ))}
                </div>
              ...
              {side === 'BUY' && (
                <div className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-5 gap-1.5">
                    {LEVERAGES.map(lv => (
                      <Button key={lv} variant={leverage === lv ? 'secondary' : 'outline'} size="sm" className="h-7 text-[11px]" onClick={() => setLeverage(lv)}>
                        {lv}x
                      </Button>
                    ))}
                  </div>
              ...
              {/* 预览 */}
              <div className="rounded-xl bg-muted/40 px-3 py-2.5 space-y-1 text-[12px]">
                <div className="flex justify-between"><span className="text-muted-foreground">成交额</span><span className="tabular-nums font-medium">{fmtNum(amount)}</span></div>
                <div className="flex justify-between"><span className="text-muted-foreground">手续费</span><span className="tabular-nums">{fmtNum(commission)}</span></div>
                {isLevBuy && <div className="flex justify-between"><span className="text-muted-foreground">借款</span><span className="tabular-nums text-amber-400">{fmtNum(amount - amount / leverage)}</span></div>}
              ...
              >
                {submitting ? <Loader2 className="w-4 h-4 animate-spin" /> : (side === 'BUY' ? '买入' : '卖出')}
              </Button>
          ...
          {held > 0 && (
            <Card>
              <CardContent className="p-4">
                <div className="text-[11px] text-muted-foreground mb-2">当前持仓</div>
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-base font-bold tabular-nums">{fmtNum(held)} <span className="text-[11px] text-muted-foreground font-normal">股</span></div>
                    <div className="text-[11px] text-muted-foreground">均价 {fmtNum(position?.avgCost ?? 0)}</div>
                  </div>
                  <div className="text-right">
                    <div className="text-[11px] text-muted-foreground">现值</div>
