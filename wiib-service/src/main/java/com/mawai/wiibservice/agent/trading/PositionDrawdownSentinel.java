 ...
 *    （基于 Executor 跨 cycle peak 记忆，A/B 看 5min 窗口，C 看整个持仓生命周期）
 * <p>
...
public class PositionDrawdownSentinel {
    ...
    public void scan() {
        ...
        for (FuturesPositionDTO p : positions) {
            ...
            // A/B 基于 sentinel 窗口快照（5min 内变化）；C 基于 Executor 跨 cycle 峰值（仓位生命周期）
            // 峰值浮盈 ≥PEAK_ATR_THRESHOLD×ATR 且 当前浮盈 ≤ peak×RATIO → 立即平
            boolean conditionC = false;
            double peakAtrMul = 0;
            double peakDrawdownPct = 0;
            BigDecimal peakProfit = DeterministicTradingExecutor.getPositionMaxProfit(p.getId());
