...
public class BlackjackController {
    ...
    public Result<BlackjackStatusDTO> getStatus(@CurrentUserId Long userId) {
        return Result.ok(blackjackService.getStatus(userId));
    }

    @PostMapping("/bet")
    @Operation(summary = "下注开局")
    public Result<GameStateDTO> bet(@CurrentUserId Long userId, @RequestBody BlackjackBetRequest request) {
    ...
    public Result<GameStateDTO> hit(@CurrentUserId Long userId) {
        return Result.ok(blackjackService.hit(userId));
    }

    @PostMapping("/stand")
    @Operation(summary = "停牌")
    public Result<GameStateDTO> stand(@CurrentUserId Long userId) {
        return Result.ok(blackjackService.stand(userId));
    }

    @PostMapping("/double")
    @Operation(summary = "加倍")
    public Result<GameStateDTO> doubleDown(@CurrentUserId Long userId) {
        return Result.ok(blackjackService.doubleDown(userId));
    }

    @PostMapping("/split")
    @Operation(summary = "分牌")
    public Result<GameStateDTO> split(@CurrentUserId Long userId) {
        return Result.ok(blackjackService.split(userId));
    }

    @PostMapping("/insurance")
    @Operation(summary = "买保险")
    public Result<GameStateDTO> insurance(@CurrentUserId Long userId) {
        return Result.ok(blackjackService.insurance(userId));
    }

    @PostMapping("/forfeit")
    @Operation(summary = "放弃牌局")
    public Result<GameStateDTO> forfeit(@CurrentUserId Long userId) {
