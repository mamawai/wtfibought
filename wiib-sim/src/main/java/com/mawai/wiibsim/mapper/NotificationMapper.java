...
public interface NotificationMapper extends BaseMapper<Notification> {
     ...
     *
     * <p>JOIN 必须是 LEFT：交易通知没有 actor_id，用 INNER JOIN 会把它们整个过滤掉。</p>
     */
    @Select("""
            SELECT n.id, n.type, n.comment_id AS commentId, n.actor_id AS actorId,
                   u.username AS actorName, n.symbol, n.side, n.quantity, n.price, n.pnl,
                   n.is_read AS isRead, n.created_at AS createdAt
            FROM notification n LEFT JOIN "user" u ON u.id = n.actor_id
    ...
    @Delete("DELETE FROM notification WHERE comment_id = #{commentId}")
    int deleteByCommentId(@Param("commentId") long commentId);

    /** 根评论被删=其子评论一并软删，指向这些子评论的通知也要清 */
    @Delete("DELETE FROM notification WHERE comment_id IN (SELECT id FROM comment WHERE root_id = #{rootId})")
