...
| ETH 窗口绩效 > baseline 20% 的窗口数 | **至少 1 个** |
...
### 2.3 参考指标（不作为硬指标，但报告必须列）

| 指标 | 说明 |
|---|---|
| 新 cell 在各窗口的交易数 vs baseline | 如果新 cell 总交易数 < baseline 的 50%，需要分析"少做"是否真的来自"过滤更严"（好）还是"信号缺失"（坏）。用户对开仓数有诉求，过度稀疏不可接受 |
...
按单跑 8 分钟算，60 个数据点约 8 小时。可以脚本批量跑，过夜完成。
