...
Jev 预测员（agent `prediction/` 包，平台用自己的 Jev key）：Jev 拍板，代码算数和执行。按局跑：局记在 `jev_prediction_run`，一局一个只有游戏钱包的账户（R1 `jev-prediction`，之后 `jev-prediction-rN`，开局各 100），/admin 关着开关才能重新开局，旧局留档。`JevPredictionRunner` 开盘后每 15 秒（起手 30…270 秒）跑一次：用当前局的账户查本回合在持的注单 → `PredictionStateWriter` 写 state，空仓持仓同一份（只有中性事实的词：时段、BTC 相对开盘均价与领先多少个正常波动、末分钟已锁定的那部分、路径与最近一分钟、Chainlink 多久没更新与 Binance 最近 10/30 秒涨跌、最近 60 秒资金与开盘以来强平、谁是热门与市场给两边的概率、两边买一卖一和含费成本、最近 30 秒两边赔率变动、最近 15 秒里 UP 中间价 3 秒内涨或跌到 10¢ 的突变——滑动窗口找两点相隔不超过 3.5 秒的最大涨、最大跌，一样大取用时短的再取后发生的，按先后写从几美分到几美分、几秒前、现在多少；涉及两边的句子两边都写，只按 UP 叙述时 Jev 偏向 DOWN；数学估计不写进 state，它比市场慢半拍，写了 Jev 会去买刚被砸的那边；价格取 Polymarket 推的 Chainlink 现货、按 Chainlink 时间戳，逐笔与强平直接读 Redis/库）→ 盘口超过 5 秒没更新、Chainlink 超过 5 秒没更新（Polymarket 上游断流，记 STALE_CHAINLINK 不算出错）、持仓没人接盘都不问不动 → `PredictionJudge` 一次请求只问一道入场题（买 UP / 买 DOWN / 不买，选项顺序固定 UP 在前，Jev 对选项位置敏感）→ `PredictionRules` 执行：Jev 选的那一项概率到 0.5 才照做，每注 5 USDT；空仓选哪边买哪边；持仓选另一边就卖掉本回合全部注单（这一格不反手），选手里这边就加注（同一回合这一边在持本金最多 10 USDT），选不买就拿着；代码只拦那边没人卖、钱包付不起、加满了；要成交的等 1 秒再看盘口，价比 Jev 看到的差不超过 3¢ 就按那时的价成交（reason 记"看到的→实际的"，页面写预计和实际），再差或那边没价记 MISSED；钱包付不起一注、也没有等结算的注单就自动关开关。R4 的措辞是拿 R3-v2 的真实 state 离线重放、按预注册的行为标准（镜像对称、冷门买入、相邻检查点换边、PASS 比例）选的，不看输赢。数学估计来自 `PredictionModel`：领先 / 剩余时间 / 波动出 Φ(z)，波动取一小时典型值和最近几分钟实际值的较大者，末分钟是收盘前 63~3 秒，进了以后已走过的均价按权重锁定；只记分、给页面当参考。每次一行落 `jev_prediction_decision`（带局号、Jev 的回答与拍板的选项和概率、盘口年龄、最近 15 秒的最大涨跌幅、数学参考优势；持仓行记在持的合计），每分钟按行所属那一局的账户回填结果与盈亏；记分看决策统计（买卖次数、手续费、实际盈亏、都拿到结算的盈亏），R1–R3 另有数学 p_model、Jev 的 p_jev、市场 p_mkt 三列 Brier。feed 收到盘口消息只记内存，每 50ms 一拍：先看要不要换回合，再把有变化的盘口写 Redis、推给页面；每秒把盘口最后更新时刻和 UP 中间价采样进 Redis（供判旧、"最近 30 秒赔率怎么动"和突变），并补推一次当前盘口；按消息自带时间戳，CLOB 盘口落后 3 秒、Chainlink 现货落后 5 秒就换一条连接。下注走 sim 的 `/internal/prediction` 通道。展示在独立页 `/jev`（`pages/JevPrediction.tsx`，组件在 `components/jev/`），按局看、旧局标已归档：左上与预测页同一张行情头卡（`PredictionHero` + `usePredictionMarket`，两页共用），左下 Jev 的交易与决策统计，R1–R3 另有按检查点的三列 Brier 与校准，右栏每 15 秒一张卡：实际动作、Jev 选了什么和把握、数学参考数、上涨概率，点开看回答条、当时盘口和最近 15 秒突变；每个回合默认只露出有动作的卡和最新一张，R1（决定题 + 后劲题）、R2（Jev 报胜率、代码下注）、R3（空仓入场题 + 持仓离场题，state 带估计）的旧卡照原样显示。题目在页面上用自己的话包装，不放英文原题。
...
│   ├── runtime/                      # 平台功能位模型分配（只剩快讯翻译主备两位，Admin 热更）
