...
模型来源：面向用户的四处（交易、对话、复盘教练、行为分析）全部 BYOK（用户自带 key，AES-GCM 加密存库），统一放在一个端点库里，按用途绑定。交易员一天自动跑几十上百轮要便宜稳，对话是按需的深度研判要强模型，所以允许各绑各的端点；只配一条时它就是全局默认。平台自己掏钱的只剩 news-translation（快讯英文译文）一个功能位（`ai_runtime_config` + `ai_model_assignment`）——它是后台批量任务，没有"当前用户"可言。behavior 的平台功能位已退休，那次调用记在用户自己的 key 上。
...
trader 每次唤醒收到三份注入：平台系统提示词（环境 / 工具 / 规格 / 护栏，只讲事实，不带交易观点）、复盘笔记（自己的教训）、学习笔记（从别人那学到的），再加主人的交易指令（方法 / 风格 / 纪律的唯一来源，用户自己写，新建时预填平台默认那套）。几份并列不合并，来源分开模型才分得清哪条是自己的教训、哪条是学来的、哪条是主人定的。trader 只读这些笔记，不关心是谁写的，以后再加一份新笔记也不用改 trader。
...
- 路由的 Jev 层（`JevRouter`）：用户在模型配置页配了 TypeSafe Jev（BYOK，一人一份 `user_jev_config`）就先问它——三道英文是非题一次问完"需不需要 market / news / trader 专家"，概率过 0.5 的进名单，都不过就是 FINISH。state 只带本轮提问与最近几条用户/助手原文，专家回填的数据不进。没配、调用失败、回包缺题都回落到上面那条浅模型路由，去重与 3 轮上限两条路共用。
