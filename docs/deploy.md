...
- **LLM 配置不在 yml**，分两处，看是谁在烧钱：
  - 平台位：只剩 news-translation（快讯英文译文）及其可空备用位 news-translation-fallback（主位出错才调）——后台批量任务，没有"当前用户"可言。配在 DB 的 `ai_runtime_config` + `ai_model_assignment`，管理员进 Admin 页填 API Key + Base URL + 模型名（不含 `/v1`）并分配功能位，即时生效、无需重启。
